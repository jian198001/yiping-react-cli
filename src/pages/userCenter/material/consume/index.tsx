// 导入 ConsumeInstock 组件
import ConsumeInstock from '../instock/ConsumeInstock';
// 导入分页查询的函数
import { page, } from '@/services/userCenter/material/consume';
  ...
  const columns: ProColumns<never, "text">[] = [...getColumns?.(formItems),
    {
      ...
      // 值类型为 option
      valueType: 'option',
      // 渲染函数，返回一个归还入库按钮，点击按钮弹出相应模态框
      render: (_, record) => [
        <>
          {(<ConsumeInstock record={record} onOk={onOk} trigger={<Button type="primary" >归还入库</Button>}></ConsumeInstock>)}
        </>
      ],
    },
  ];
