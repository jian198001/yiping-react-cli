...
export default () => {
  ...
  const columns: ProColumns<never, "text">[] = [
    ...
    },
    {
      ...
      render: (_, record) => [
        <>
          {(!record?.tradeState || record?.tradeState === "edit") && (
            <Update
              key={record?.id}
              trigger={<Button type="primary">编 辑</Button>}
              id={record?.id}
              onOk={onOk}
            ></Update>
          )}
          ...
          {record?.tradeState === "submit" && (
            <PurchaseInstock
              id={record?.id}
              trigger={<Button type="primary">入 库</Button>}
              onOk={onOk}
            />
          )}
          {(!record?.tradeState || record?.tradeState === "edit") && (
  ...
  const handleSubmit = async (item: any) => {
    Modal?.confirm?.({
      title: "操作提示",
      content: `是否确定提交标题为${item?.title}的信息?提交后将不可再次编辑`,
      onOk() {
        // 确认操作的回调函数
