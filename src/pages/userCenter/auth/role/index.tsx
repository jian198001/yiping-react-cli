...
import { page, del } from '@/services/userCenter/auth/role';
  ...
  const columns: ProColumns<never, "text">[] = [ ...getColumns?.(formItems), 
 
    {
      ...
      render: (_, record) => [
            ...
            <Update key={record.id}   trigger={<Button type='primary' disabled={true}>
              编 辑
  ...
  return (
    <PageContainer>
      ...
      <ProTable
        ...
        toolBarRender={() => [<> <Update  trigger={<Button type="primary" disabled={true}> 新 建 </Button>}    id={uid}      onOk={onOk}  ></Update></> ]}
        request={page}
        columns={columns}
        rowSelection={{
