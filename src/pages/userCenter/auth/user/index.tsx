...
export default () => {
...

  {
    ...
    render: (_, record) => [
      ...
      <Update key={record?.id} trigger={<Button type='primary' disabled={record?.username === 'admin' || !isAdmin() } >
        编 辑
      </Button>} id={record?.id} onOk={actionRef?.current?.reload}></Update>,
      <Button disabled={record?.username === 'admin' || !isAdmin() } onClick={()=>{
      ...
      , <Button danger onClick={() => {
        handleRemove(record, );
      }} disabled={record?.username === 'admin' || !isAdmin() } >删 除</Button>
    ],
  },
  ];
  ...
  const handleRemove = useCallback(
    async (item: any) => {
      ...
      Modal?.confirm?.({
        title: '操作提示', content: `是否确定删除用户名为${item.username}的信息?删除后将不可恢复`, onOk() {
          // 确认操作的回调函数
  ...
  const handleResetPwd = useCallback(
    async (item: any) => {
      ...
      Modal?.confirm?.({
        title: '操作提示', content: `是否确定重置用户名为${item.username}的密码?重置后此用户密码将为初始密码aaaa1111`, onOk() {
          // 确认操作的回调函数
  ...
  return (
    <PageContainer>
      ...
      <ProTable
        ...
        toolBarRender={() => [<> <Save trigger={<Button type="primary" disabled={!isAdmin() }> 新 建 </Button>} id={uid} onOk={onOk}  ></Save></>]}
        request={page}
        columns={columns}
        rowSelection={{
